...
export class CargaMesaComponent implements OnInit, OnDestroy {
  ...
  onSubmitMesa(): void {
    if (this.formMesa.valid) {
      this.mesa.numeroMesa = this.formMesa.controls.numeroMesa.value;
      this.mesa.cantidadComensales = this.formMesa.controls.cantidadComensales.value;
      this.mesa.tipoMesa = this.formMesa.controls.tipoMesa.value;
      // this.mesa.foto = ....
      this.mesa.qr = this.formMesa.controls.qr.value;
      alert('Envío Mesa');
    } else {
  ...
  public mostrarError(control: string): string {
    ...
    switch (control) {
      ...
      case 'tipoMesa':
        ...
        break;
      case 'qr':
        if (this.formMesa.controls.qr.hasError('required')) {
          retorno = 'Debe ingresar el código QR de la mesa';
        } else {
          retorno = 'Error inesperado con el código QR de la mesa';
        }
        break;
    }
